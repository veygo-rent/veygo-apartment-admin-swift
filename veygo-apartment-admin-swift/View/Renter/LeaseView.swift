...
struct LeaseView: View {
        ...
        ScrollView {
            VStack(spacing: 16) {
                if isLoading {
                    ...
                    LeasePDFDocumentContainerView(pdfData: loadedPdfData)
                ...
                if renter != nil {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Picker("Action", selection: $selectedAction) {
                            Text("Declined").tag(VerifyLeaseAction.declined)
                            Text("Approved").tag(VerifyLeaseAction.approved)
                        }
    ...
    private func requestNextRenter() {
        ...
        Task {
            await ApiCallActor.shared.appendApi { token, userId in
                await loadNextRenterNeedLeaseVerifyAsync(token, userId)
            }
        }
    }

    private func submitVerification(_ action: VerifyLeaseAction) {
        ...
        if action == .declined {
            ...
            if trimmedReason.isEmpty {
                ...
                alertMessage = "Please provide a reason to decline lease."
                showAlert = true
                return
            }
        }
        ...
        if action == .approved {
            ...
            if street.isEmpty || city.isEmpty || state.isEmpty || zipcode.isEmpty {
                ...
                alertMessage = "Street, city, state, and zipcode are required for lease approval."
                showAlert = true
                return
            }
        }
    ...
    @ApiCallActor
    private func loadNextRenterNeedLeaseVerifyAsync(_ token: String, _ userId: Int) async -> ApiTaskResponse {
        defer {
            Task { @MainActor in
                isLoading = false
            }
    ...
    @ApiCallActor
    private func verifyLeaseAsync(
        _ token: String,
        _ userId: Int,
        renterId: Int,
        action: VerifyLeaseAction
    ...
    private func loadDocumentAsync(_ documentURL: URL) async {
        do {
...
private struct LeasePDFDocumentContainerView: UIViewRepresentable {
    let pdfData: Data
